...
def checkLineLength(event):
    ...
    for i in range(1, lenghtOfLines+1):
        linenum.insert(END, str(i).rjust(3, '0') + ' - ')
        if i != lenghtOfLines:
            linenum.insert(END, "\n")
...
scroll.pack(side=RIGHT, fill=Y)
linenum = Text(window, font=commonfont, width=6,state=DISABLED, highlightthickness=0, borderwidth=0)
linenum.pack(side=LEFT, anchor=NW, expand=False, fill=Y)
...
def hight(event=None):
    ...
    if event.char == " " or event.keysym == "BackSpace" and suggestion_pos == ("", ""):
        res = re.findall(r'\w+', editor.get ("1.0", END))
        setOfWords = set(res)

    for x in highlight:
        start = editor.index(INSERT).split(".")[0] + ".0"
        end = "insert lineend +1c"
        editor.tag_remove(x[0], start, end)
        while start != end:
            try:
                    countVar = StringVar()
...
def stop_separators():
    ...
    editor.config(autoseparator=False)

def start_separators():
    editor.config(autoseparator=True)
    editor.edit_separator()
...
def onButtonPress (event:Event):
    ...
    if event.char == ")":
        if suggestion_pos != ("", ""):
            ...
            editor.delete (suggestion_pos[0] + "+1c", suggestion_pos[1] + "+1c")
            suggestion_pos = ("", "")
        nextChar = editor.get("insert", "insert+1c")
        if nextChar == ")":
            editor.mark_set("insert", editor.index("insert") + "+1c")
            return "break"
    ...
    if event.char == " ":
        ...
        suggestion = suggest (list(set(suggestionsList) | setOfWords), word)
        if suggestion != "":
...
editor.config(bg=backgroundcolor, fg=forgroundcolor, insertbackground=forgroundcolor,
    highlightthickness=0, borderwidth=0, tabs=tab, undo=True
)
linenum.config(bg=backgroundcolor, fg="gray", highlightthickness=0, borderwidth=0)
...
editor.bind("<Command-s>", byctrls)
editor.bind("<Command-z>", lambda e: editor.edit_undo ())
editor.bind("<Command-y>", redoCommand)
editor.bind("<Command-c>", lambda e: editor.event_generate("<<Copy>>"))
editor.bind("<Command-v>", lambda e: editor.event_generate("<<Past>>"))
editor.bind("<Command-BackSpace>", lambda e: editor.delete ("insert linestart", "insert lineend"))
editor.bind("<KeyPress>", onButtonPress)
editor.bind("<KeyRelease>", hight)
editor.bind("<Command-+>", incfont)
editor.bind("<Command-minus>", decfont)
window.bind("<F11>",
...
editor.tag_configure("current_line", background=lineMarkColor)
